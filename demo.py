...
def parse_args():
	...
	parser.add_argument(
		'--soft_nms', help='whether use soft nms', action='store_true')
	parser.add_argument(
		'--cascade', help='whether use cascade', action='store_true')
...
if __name__ == '__main__':
	...
	if args.cascade:
		if args.net == 'detnet59':
			fpn = detnet_cascade(classes, 59, pretrained=False,
		...
		if args.net == 'detnet59':
			fpn = detnet_noncascade(
				classes, 59, pretrained=False, class_agnostic=args.class_agnostic)
		else:
			print("network is not defined")
			pdb.set_trace()
	...
	for i in range(num_images):
		...
		for j in xrange(1, len(classes)):
			...
			if vis:
				im2show = vis_detections(
					im2show, classes[j], cls_dets, thresh=0.5)
		...
		if vis:
			...
			result_path = os.path.join(
				args.result_dir, imglist[i][:-4] + "_det.jpg")
